// Type for test responses (compatible with Prisma model)
...
export const calculateRorschachStats = (responses: TestResponseInput[]): RorschachStats => {
  ...
  responses.forEach(r => {
    const cont = r.contentCategories?.trim();
    if (!cont) return;
